...
export function getNumberOfRecipes(categoryId) {
  ...
  for(let i = 0; i < recipes.length; i++) {
    for(let j = 0; j < recipes[i].items.length; j++) {
      recipesArray.push(recipes[i].items[j]);
    }
  }
...
export function getRecipesByRecipeName(recipeName, productArray) {
